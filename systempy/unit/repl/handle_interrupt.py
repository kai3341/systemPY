...
class ReplHandlers(NamedTuple):
    handle_interrupt: Callable[[Any], None]
    setup_completer: Callable[[Any], None]


def __fallback(unit: Any) -> None: ...


@cache
def get_repr_handlers() -> ReplHandlers:
    try:
        import ctypes
        import readline
        import rlcompleter
        ...
        return ReplHandlers(handle_interrupt__readline, setup_completer__readline)
        ...
        return ReplHandlers(__fallback, __fallback)
